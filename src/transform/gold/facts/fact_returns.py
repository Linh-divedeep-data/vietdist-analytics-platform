...
def build_fact_returns(
    ...
    via SCD2 versions, stamp audit columns — plain left join on employee_id would fan out
    (dim_employees has multiple rows per employee_id, one per SCD2 version), so join_employee_asof()
    ...
    result = join_employee_asof(result, dim_employees_df, "return_date")
