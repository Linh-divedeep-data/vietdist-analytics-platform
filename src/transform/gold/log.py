...
Silver's silver_log.jsonl contract so all 3 layers log the same way."""
...
def write_gold_log(record: dict, out_dir: str) -> str:
    """Persist one Gold log record to out_dir/gold_log.jsonl, appending to any
    ...
    path = os.path.join(out_dir, "gold_log.jsonl")
