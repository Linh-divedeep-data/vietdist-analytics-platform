...
if not FOLDER_ID:
    # Module-level (không phải chỉ trong __main__): fail-fast ngay lúc import,
    # bất kể module này được chạy trực tiếp hay import từ nơi khác (main.py).
    # Thiếu check này, list_files_in_folder(None) sẽ crash HttpError 404 khó hiểu
    # thay vì báo rõ nguyên nhân là thiếu cấu hình .env.
...
def list_files_in_folder(folder_id):
    ...
    Duyệt hết các trang kết quả (Drive API mặc định trả về tối đa ~100
    file/trang, cần nextPageToken để lấy tiếp) — nếu không, folder nhiều
    hơn 1 trang sẽ bị bỏ sót file mà không báo lỗi gì.
    """
